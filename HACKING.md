...
In addition to `TEST_OS`, the `Makefile` supports also `TEST_SCENARIO` to specify
which branch of [subscription-manager.git](https://github.com/candlepin/subscription-manager) to test.
If not specified, the default is the distribution packaged version.
The `TEST_SCENARIO=devel` means that the distribution packaged
subscription-manager is tested but also collects code coverage and runs pixel tests.
For compatibility, setting `TEST_OS=system` also runs tests against
the distribution packaged version.
